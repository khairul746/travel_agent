...
class ToolEventCollector(BaseCallbackHandler):
    ...
    def on_tool_start(self, serialized, input_str, **kwargs):
        self.events.append({"name": serialized.get("name"), "input": input_str})
...
If a session_id is returned, keep it for follow-up tool calls but don't show it in the message content."""
    ...
    async def main():
        ...
        while True:
            ...
            try:
                ...
                print("Artifacts:")
                print(collector.events[-1]["output"].content if collector.events else "No artifacts")
                if input_message == "quit" or input_message == "exit":
                    await agent.ainvoke({"messages": "Close the session"}, config=config)
                    break
